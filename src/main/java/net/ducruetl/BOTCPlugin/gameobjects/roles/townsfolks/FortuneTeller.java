    ...
    public void sendInfoMessage(GamePlayer player, GamePlayer firstSelectedPlayer,
        ...
        if (impFound) {
            final Component message = MiniMessage.miniMessage().deserialize(
                "<blue>Le diablotin se trouve entre</blue> <yellow><bold><firstname></bold></yellow> <blue>et</blue> <yellow><bold><secondname></bold></yellow>",
                Placeholder.unparsed("firstname", firstSelectedPlayer.getPlayer().getName()),
                Placeholder.unparsed("secondname", secondSelectedPlayer.getPlayer().getName())
            );
            player.getPlayer().sendMessage(message);
        } else {
            final Component message = MiniMessage.miniMessage().deserialize(
                "<blue>Le diablotin n'est pas présent entre</blue> <yellow><bold><firstname></bold></yellow> <blue>et</blue> <yellow><bold><secondname></bold></yellow>",
                Placeholder.unparsed("firstname", firstSelectedPlayer.getPlayer().getName()),
                Placeholder.unparsed("secondname", secondSelectedPlayer.getPlayer().getName())
            );
            player.getPlayer().sendMessage(message);
